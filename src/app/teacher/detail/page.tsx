...
const DetailTeacher = () => {
    ...
    useEffect(() => {
        ...
        const formattedData: string[][] = sortStudents(
          students,
          monitorCode,
          setMonitor
        );
        ...
        const formattedSession: string[][] = sessions.map((item: any) => {
          if (item.no === 1) {
  ...
  useEffect(() => {
    const fetchSession = async () => {
      ...
      try {
        ...
        const formattedData: string[][] = response.data.map(
  ...
  const sessionTableData = sessionData
    .map((row) => row.slice(3))
    .sort((a, b) => a[0] - b[0]);
  ...
  const handleSelectRollCaller = (index: number) => {
    const rollCallerSelectList = sortRollcallerToTop(
      selectOptions,
      rollCaller.name
    );
              ...
              <RoundedButton
                ...
                onClick={() => {
                  ...
                  setMessage({
                    ...
                    title: "Confirmation",
                    description:
                      "Are you sure you want to update class's information?",
                  });
                }}
              ></RoundedButton>
            </div>
